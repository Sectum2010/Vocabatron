...
Vocabatron 是本机运行的私人课程库与填字应用。它保留原文字段和来源证据，由本机 `gemma4:31b` 选择已有候选，再冻结线索。完整布局候选先经过独立结构、全历史和排版核验；需要继续搜索时由 CP-SAT 检查完整位置域。最终两页 PDF 另行独立验证。
...
上传只接收有大小限制的原始字节并排队。先读取原生文字和位置证据；扫描页或未解决的结构进入受监督的本机 OCR 恢复。每课通过确定性的字段、候选和来源归属核验后发布，不由模型改写词表。多课文档部分失败时保留其他已验证课程，并显示具体问题。安装、离线边界与恢复规则见 [文档恢复说明](docs/document-recovery.md)。
...
计算永远低于其他应用。工作进程使用 SCHED_IDLE、Nice 19 和 idle I/O，CPU 并发随空闲容量调整。专用 cgroup 保留内存硬边界和零交换；不再永久限速为两个 CPU 或固定磁盘吞吐。采样过期或 I/O 繁忙时停止接纳下一单元，当前有限单元完成或检查点让位；内存不足或明确 hold 会暂停自有工作。共享 Ollama 不属于本应用 cgroup，客户端优先级不能即时抢占 GPU 推理。
