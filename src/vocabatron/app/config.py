...
class ResourcePolicy(BaseModel):
    ...
    cpu_threads: int = Field(default=8, ge=1, le=16)
    reserved_cpu_cores: int = Field(default=2,ge=1,le=8)
    search_slots: int = Field(default=1, ge=1, le=2)
    threads_per_search: int = Field(default=8, ge=1, le=8)
    document_slots: int = Field(default=1, ge=1, le=2)
    memory_max_bytes: int = Field(default=44*1024**3, ge=2*1024**3, le=64*1024**3)
    memory_high_bytes: int = Field(default=30*1024**3,ge=1024**3,le=48*1024**3)
    ...
    external_cpu_start_percent: float = Field(default=65, gt=0, le=85)
    external_cpu_stop_percent: float = Field(default=80, gt=0, le=95)
    cpu_work_during_gpu_activity: bool = True
    cpu_psi_percent: float = Field(default=5, gt=0, le=10)
    cpu_pressure_min_external_percent: float = Field(default=60,ge=0,le=90)
    memory_psi_percent: float = Field(default=0.5, gt=0, le=1)
    io_psi_percent: float = Field(default=2, gt=0, le=5)
    idle_window_seconds: float = Field(default=5, ge=1, le=120)
...
class AppConfig(BaseModel):
    ...
    document_limits: Limits = Limits(pages=256, characters=2_000_000,address_bytes=12*1024**3)
